...
"""
Correctness tests for the SenseNova-U1 native-Ascend fused RMSNorm + 3D RoPE.

``npu_qk_norm_rope`` replaces the model's PyTorch fallback chain (4x RMSNorm +
3x Neox RoPE) with ``torch_npu.npu_rms_norm`` / ``torch_npu.npu_rotary_mul``.
Verifies it stays numerically equivalent to the PyTorch reference on the E2E
shapes. Mirrors ``test_sensenova_triton_kernel.py`` — same math, same reference,
NPU device instead of CUDA.
"""
...
try:
    from vllm_omni.diffusion.models.sensenova_u1.npu_fused_rmsnorm_rope import npu_qk_norm_rope

    _NPU_FUSED_AVAILABLE = True
except ImportError:
    _NPU_FUSED_AVAILABLE = False

npu_fused_available = pytest.mark.skipif(not _NPU_FUSED_AVAILABLE, reason="NPU fused op unavailable")
...
def assert_close_with_error_stats(
    ...
    *,
    ...
    try:
        ...
        # Stats on CPU: torch.quantile has spotty NPU support and would mask
        # the real failure with an unrelated error.
        abs_diff = (actual.float() - expected.float()).abs().flatten().cpu()
        tolerance = atol + rtol * expected.float().abs().flatten().cpu()
...
def reference_kernel(
    ...
    """PyTorch reference matching the model's math."""
...
def test_fused_qk_norm_rope_matches_reference(seq_len: int, dtype: torch.dtype, atol: float, rtol: float):
    ...
    out_q, out_k = npu_qk_norm_rope(*call_args, EPS)
